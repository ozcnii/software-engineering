...
- **Игрок** — выбирает лабиринт из списка и проходит его (вручную стрелками или автоматически алгоритмами волновой алгоритм / метод правой руки)
...
state "Создание лабиринта" as СозданиеЛабиринта {
  ...
  state "Шаг 2.\nРедактирование на холсте" as Редактор {
    Редактор : entry / сгенерировать шаблон\n        с периметром стен
    ...
    Редактор : нажата кнопка "Сгенерировать лабиринт" /\n     сгенерировать лабиринт\n     выбранным алгоритмом
  }
...
state "Просмотр сведений\nо системе" as Система {
  Система : entry / отобразить страницу\n        "О системе"
...
state "Прохождение лабиринта" as Прохождение {
  ...
  state "Автоматическое решение" as АвтоРешение {
    ...
    АвтоРешение : do / выбрать алгоритм поиска пути (волновой алгоритм / метод правой руки)
...
- Поток: система открывает форму шага 1 «Параметры» (поля: название, ширина, высота, тема, вход/выход, алгоритм генерации) → админ кликает в поле «Название» → вводит → нажимает «Далее — вход и выход» → система проверяет заполненность и уникальность поля.
